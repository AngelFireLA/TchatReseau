...
from avec_interface.utils import show_text, color_dict, HEIGHT, WIDTH, load_config, show_multiline_text, clean_text, \
    calculate_text_size, get_linesize, wrap_text
...
def main(current_client: Client, screen):
    ...
    text_area = pygame.Rect(WIDTH // 2 - 700, HEIGHT - 200 - 20, 1400, 200)
    contour_area = pygame.Rect(text_area.x - 5, text_area.y - 5, text_area.width + 10, text_area.height + 10)
    message_font_size = 30
    lowest_message_y = contour_area.y - 5 - get_linesize(
        message_font_size)  # how low can the bottom of the last message go
    highest_message_y = 30  # how high can the top of the first message go
    ...
    while True:
        ...
        if current_message_index < len(current_client.messages):
            ...
            for message in messages_to_iterate_over:
                ...
                total_width, total_height = calculate_text_size(message_text, message_font_size, WIDTH-100)
                current_message_y += total_height + 10
                show_multiline_text(screen, 50, current_message_y-total_height, message_text, message_font_size, color_dict["text"], WIDTH-100)
        pygame.display.flip()